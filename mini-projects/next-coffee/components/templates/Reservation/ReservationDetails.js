...
function ReservationDetail() {
  const reserveReducer = (reserveTableData, action) => {
    switch (action.type) {
      case "name": {
        return { ...reserveTableData, name: action.payload };
      }
      case "email": {
        return { ...reserveTableData, email: action.payload };
      }
      case "date": {
        return { ...reserveTableData, date: action.payload };
      }
      case "time": {
        return { ...reserveTableData, time: action.payload };
      }
      case "person": {
        return { ...reserveTableData, person: action.payload };
      }
      default: {
        return reserveTableData;
      }
    }
  };

  const [reserveTableData, dispatch] = useReducer(reserveReducer, {
    name: "",
    email: "",
    date: "",
    time: "",
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://localhost:4000/reserve", {
      ...
      body: JSON.stringify(reserveTableData),
    });
